    ...
    return (
        <div className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group">
            ...
            >
                ...
                {product.isLocal && (
                    <div className="absolute top-2 left-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-full flex items-center gap-1 shadow-sm">
                        <svg className="w-3 h-3 text-kaiser-green-600" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                        <span className="text-[10px] font-bold text-kaiser-green-700 uppercase tracking-wider">Local</span>
                    </div>
                )}
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
                <span className="text-lg font-bold text-kaiser-green-700">${product.price.toFixed(2)}</span>
                <button
                    onClick={handleAdd}
                    className={`p-2 rounded-full transition-all active:scale-95 text-white ${isAdded ? 'bg-kaiser-accent' : 'bg-gray-100 text-kaiser-green-700 hover:bg-kaiser-green-100'
                        }`}
                >
